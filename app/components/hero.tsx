...
export function Hero() {
  ...
  const artworkX = useTransform(() => x.get() * 30 - scrollYProgress.get() * 64);
  const artworkY = useTransform(() => y.get() * 20 - scrollYProgress.get() * 48);
  const artworkRotate = useTransform(() => x.get() * 0.7 + scrollYProgress.get() * 1.5);
  ...
  useEffect(() => {
    if (reducedMotion) return;
    // The print leads a single typesetting settle. Text stays readable throughout.
    const entrance = animate([
      ["[data-hero-art-entry]", { x: [54, 0], y: [18, 0] }, { duration: 1.15, ease: editorialEase }],
    ...
    return () => {
      entrance.stop();
      section.current?.querySelectorAll<HTMLElement>("[data-hero-art-entry], [data-hero-line], [data-hero-rule]").forEach((element) => {
  ...
  return (
    ...
    >
      <div className="hero-art-scene" data-artwork="hero" aria-hidden="true">
        <m.div
          className="hero-art-plane"
          style={reducedMotion ? undefined : { x: artworkX, y: artworkY, rotate: artworkRotate }}
        >
          <div className="hero-art-entry" data-hero-art-entry>
            <img className="hero-art-media" src="/images/art/hero-poster.webp" alt="" width="1586" height="992" decoding="async" fetchPriority="low" />
          </div>
        </m.div>
      </div>
