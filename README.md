...
The generated prints in `public/images/art` frame the real portrait and project screenshots. `HeroArt` assembles four colored paths from the original convergence print using SVG masks and a finite Motion timeline; the hub stays intact, then the complete artwork holds. Artwork entrances wait for image decoding and visibility, play automatically once per mount, and have no playback controls. The hero artwork plane stays fixed; the portrait ink responds to pointer and scroll depth. `ProjectArt` uses cropped planes for Send's horizontal flow, Shenanigan's stepped entrance, BrightID's alignment, and open-source branching. Keep the media dimensions, decorative semantics, static fallback, and reduced-motion behavior when changing the media format.
