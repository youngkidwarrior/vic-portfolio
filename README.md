...
The generated prints in `public/images/art` frame the real portrait and project screenshots. `HeroArt` assembles four colored paths from the original convergence print using SVG masks and a finite Motion timeline; the hub stays intact, then the complete artwork holds. Its entrance waits for image decoding and visibility. Pointer and scroll depth move the completed composition as one plane. `ProjectArt` uses cropped planes for Send's horizontal flow, Shenanigan's stepped entrance, BrightID's alignment, and open-source branching. Keep the media dimensions, decorative semantics, static fallback, and reduced-motion behavior when changing the media format.
